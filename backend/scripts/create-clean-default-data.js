// Create clean default data with fewer elections for better testing
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Generate clean default data
const cleanDefaultData = {
  superadmins: [],
  
  admins: [
    {
      "id": "admin-001",
      "username": "admin1123",
      "email": "admin1123@votingsystem.com",
      "password": "$2b$10$bPYyvJ99TicH6hviI6bZ1OQ8BM60xF/KU29gpEl5aQZTKYXM5JkAS",
      "role": "admin",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T20:16:48.000Z"
    },
    {
      "id": "admin-002",
      "username": "admin2",
      "email": "admin2@votingsystem.com",
      "password": "$2b$10$865cIoKXjrNw1Bit5OQyHu4/OHpmpNwgSDEEkrC.VN5g4KglkardW",
      "role": "admin",
      "created_at": "2025-07-28T20:20:51.000Z",
      "updated_at": "2025-07-28T20:20:51.000Z"
    },
    {
      "id": "admin-003",
      "username": "admin3",
      "email": "admin3@votingsystem.com",
      "password": "$2b$10$bKiewgfLtQx8Pi73XXawMOBpVIXUc0E3fUtR9rTxaOj9Y1UntPpdy",
      "role": "admin",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "superadmin-001",
      "username": "superadmin",
      "email": "superadmin@votingsystem.com",
      "password": "$2b$10$iEKbBdMsWDP789SUOCvu1ueZtKvMzIuNlOFLf8dMLTMl9gkZnTVT2",
      "role": "superadmin",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T20:17:20.000Z"
    }
  ],
  
  departments: [
    {
      "id": "dept-001",
      "name": "Computer Science",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "dept-002",
      "name": "Information Technology",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "dept-003",
      "name": "Civil Engineering",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "dept-004",
      "name": "Mechanical Engineering",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "dept-005",
      "name": "Electrical Engineering",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "dept-006",
      "name": "Business Administration",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "dept-007",
      "name": "Accountancy",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "dept-008",
      "name": "Marketing Management",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "dept-009",
      "name": "Psychology",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "dept-010",
      "name": "English Literature",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "dept-011",
      "name": "Mathematics",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "dept-012",
      "name": "Biology",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    }
  ],
  
  courses: [
    {
      "id": "course-001",
      "name": "BS Computer Science",
      "departmentId": "dept-001",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "course-002",
      "name": "BS Information Technology",
      "departmentId": "dept-002",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "course-003",
      "name": "BS Civil Engineering",
      "departmentId": "dept-003",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "course-004",
      "name": "BS Mechanical Engineering",
      "departmentId": "dept-004",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "course-005",
      "name": "BS Electrical Engineering",
      "departmentId": "dept-005",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "course-006",
      "name": "BS Business Administration",
      "departmentId": "dept-006",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "course-007",
      "name": "BS Accountancy",
      "departmentId": "dept-007",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "course-008",
      "name": "BS Marketing Management",
      "departmentId": "dept-008",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "course-009",
      "name": "BS Psychology",
      "departmentId": "dept-009",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "course-010",
      "name": "BA English Literature",
      "departmentId": "dept-010",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "course-011",
      "name": "BS Mathematics",
      "departmentId": "dept-011",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "course-012",
      "name": "BS Biology",
      "departmentId": "dept-012",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    }
  ],
  
  positions: [
    {
      "id": "pos-001",
      "name": "President",
      "voteLimit": 1,
      "displayOrder": 1,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "pos-002",
      "name": "Vice President",
      "voteLimit": 1,
      "displayOrder": 2,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "pos-003",
      "name": "Secretary",
      "voteLimit": 1,
      "displayOrder": 3,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "pos-004",
      "name": "Treasurer",
      "voteLimit": 1,
      "displayOrder": 4,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "pos-005",
      "name": "Auditor",
      "voteLimit": 1,
      "displayOrder": 5,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "pos-006",
      "name": "Public Relations Officer",
      "voteLimit": 1,
      "displayOrder": 6,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "pos-007",
      "name": "Business Manager",
      "voteLimit": 1,
      "displayOrder": 7,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    }
  ],
  
  candidates: [
    {
      "id": "cand-001",
      "name": "John Smith",
      "positionId": "pos-001",
      "departmentId": "dept-001",
      "courseId": "course-001",
      "photoUrl": null,
      "description": "Experienced leader with strong communication skills",
      "displayOrder": 1,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "cand-002",
      "name": "Sarah Johnson",
      "positionId": "pos-001",
      "departmentId": "dept-002",
      "courseId": "course-002",
      "photoUrl": null,
      "description": "Dedicated student leader with innovative ideas",
      "displayOrder": 2,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "cand-003",
      "name": "Michael Brown",
      "positionId": "pos-002",
      "departmentId": "dept-003",
      "courseId": "course-003",
      "photoUrl": null,
      "description": "Organized and detail-oriented leader",
      "displayOrder": 1,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "cand-004",
      "name": "Emily Davis",
      "positionId": "pos-002",
      "departmentId": "dept-004",
      "courseId": "course-004",
      "photoUrl": null,
      "description": "Collaborative leader with strong teamwork skills",
      "displayOrder": 2,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "cand-005",
      "name": "David Wilson",
      "positionId": "pos-003",
      "departmentId": "dept-005",
      "courseId": "course-005",
      "photoUrl": null,
      "description": "Excellent record keeper and communicator",
      "displayOrder": 1,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "cand-006",
      "name": "Lisa Anderson",
      "positionId": "pos-004",
      "departmentId": "dept-006",
      "courseId": "course-006",
      "photoUrl": null,
      "description": "Financial management expert with attention to detail",
      "displayOrder": 1,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "cand-007",
      "name": "Robert Taylor",
      "positionId": "pos-005",
      "departmentId": "dept-007",
      "courseId": "course-007",
      "photoUrl": null,
      "description": "Transparency and accountability advocate",
      "displayOrder": 1,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "cand-008",
      "name": "Jennifer Martinez",
      "positionId": "pos-006",
      "departmentId": "dept-008",
      "courseId": "course-008",
      "photoUrl": null,
      "description": "Creative communicator with strong public relations skills",
      "displayOrder": 1,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "cand-009",
      "name": "Christopher Lee",
      "positionId": "pos-007",
      "departmentId": "dept-009",
      "courseId": "course-009",
      "photoUrl": null,
      "description": "Strategic thinker with business development experience",
      "displayOrder": 1,
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    }
  ],
  
  voters: [
    {
      "name": "Alice Johnson",
      "email": "alice.johnson@student.edu",
      "studentId": "2024-00001",
      "hasVoted": false,
      "departmentId": "dept-001",
      "courseId": "course-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "name": "Bob Smith",
      "email": "bob.smith@student.edu",
      "studentId": "2024-00002",
      "hasVoted": false,
      "departmentId": "dept-002",
      "courseId": "course-002",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "name": "Carol Davis",
      "email": "carol.davis@student.edu",
      "studentId": "2024-00003",
      "hasVoted": false,
      "departmentId": "dept-003",
      "courseId": "course-003",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "name": "David Wilson",
      "email": "david.wilson@student.edu",
      "studentId": "2024-00004",
      "hasVoted": false,
      "departmentId": "dept-004",
      "courseId": "course-004",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "name": "Eva Brown",
      "email": "eva.brown@student.edu",
      "studentId": "2024-00005",
      "hasVoted": false,
      "departmentId": "dept-005",
      "courseId": "course-005",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    }
  ],
  
  elections: [
    {
      "id": "election-001",
      "title": "Student Council Election 2024",
      "description": "Annual election for the Student Council Executive Board. All registered students are eligible to vote.",
      "startTime": "2025-08-01T08:00:00.000Z",
      "endTime": "2025-08-08T18:00:00.000Z",
      "status": "pending",
      "created_by": "superadmin-001",
      "created_at": "2025-07-28T19:48:24.000Z",
      "updated_at": "2025-07-28T19:48:24.000Z"
    }
  ],
  
  election_positions: [
    {
      "id": "ep-001",
      "electionId": "election-001",
      "positionId": "pos-001",
      "created_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "ep-002",
      "electionId": "election-001",
      "positionId": "pos-002",
      "created_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "ep-003",
      "electionId": "election-001",
      "positionId": "pos-003",
      "created_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "ep-004",
      "electionId": "election-001",
      "positionId": "pos-004",
      "created_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "ep-005",
      "electionId": "election-001",
      "positionId": "pos-005",
      "created_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "ep-006",
      "electionId": "election-001",
      "positionId": "pos-006",
      "created_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "ep-007",
      "electionId": "election-001",
      "positionId": "pos-007",
      "created_at": "2025-07-28T19:48:24.000Z"
    }
  ],
  
  election_candidates: [
    {
      "id": "ec-001",
      "electionId": "election-001",
      "candidateId": "cand-001",
      "created_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "ec-002",
      "electionId": "election-001",
      "candidateId": "cand-002",
      "created_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "ec-003",
      "electionId": "election-001",
      "candidateId": "cand-003",
      "created_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "ec-004",
      "electionId": "election-001",
      "candidateId": "cand-004",
      "created_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "ec-005",
      "electionId": "election-001",
      "candidateId": "cand-005",
      "created_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "ec-006",
      "electionId": "election-001",
      "candidateId": "cand-006",
      "created_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "ec-007",
      "electionId": "election-001",
      "candidateId": "cand-007",
      "created_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "ec-008",
      "electionId": "election-001",
      "candidateId": "cand-008",
      "created_at": "2025-07-28T19:48:24.000Z"
    },
    {
      "id": "ec-009",
      "electionId": "election-001",
      "candidateId": "cand-009",
      "created_at": "2025-07-28T19:48:24.000Z"
    }
  ]
};

// Helper function to handle undefined values
function sanitizeValue(value) {
  return value === undefined ? null : value;
}

export async function seedWithCleanData(connection) {
  console.log('🌱 Seeding database with clean default data...');
  
  try {
    // Insert admins
    if (cleanDefaultData.admins.length > 0) {
      console.log('👨‍💼 Inserting admins...');
      for (const admin of cleanDefaultData.admins) {
        await connection.execute(
          'INSERT IGNORE INTO admins (id, username, email, password, created_at) VALUES (?, ?, ?, ?, ?)',
          [
            sanitizeValue(admin.id), 
            sanitizeValue(admin.username), 
            sanitizeValue(admin.email), 
            sanitizeValue(admin.password), 
            sanitizeValue(admin.created_at)
          ]
        );
      }
    }
    
    // Insert departments
    if (cleanDefaultData.departments.length > 0) {
      console.log('🏢 Inserting departments...');
      for (const dept of cleanDefaultData.departments) {
        await connection.execute(
          'INSERT IGNORE INTO departments (id, name, created_by, created_at) VALUES (?, ?, ?, ?)',
          [
            sanitizeValue(dept.id), 
            sanitizeValue(dept.name), 
            sanitizeValue(dept.created_by), 
            sanitizeValue(dept.created_at)
          ]
        );
      }
    }
    
    // Insert courses
    if (cleanDefaultData.courses.length > 0) {
      console.log('📚 Inserting courses...');
      for (const course of cleanDefaultData.courses) {
        await connection.execute(
          'INSERT IGNORE INTO courses (id, name, departmentId, created_by, created_at) VALUES (?, ?, ?, ?, ?)',
          [
            sanitizeValue(course.id), 
            sanitizeValue(course.name), 
            sanitizeValue(course.departmentId), 
            sanitizeValue(course.created_by), 
            sanitizeValue(course.created_at)
          ]
        );
      }
    }
    
    // Insert positions
    if (cleanDefaultData.positions.length > 0) {
      console.log('🏛️ Inserting positions...');
      for (const position of cleanDefaultData.positions) {
        await connection.execute(
          'INSERT IGNORE INTO positions (id, name, voteLimit, displayOrder, created_at) VALUES (?, ?, ?, ?, ?)',
          [
            sanitizeValue(position.id), 
            sanitizeValue(position.name), 
            sanitizeValue(position.voteLimit || 1), 
            sanitizeValue(position.displayOrder || 0), 
            sanitizeValue(position.created_at)
          ]
        );
      }
    }
    
    // Insert candidates
    if (cleanDefaultData.candidates.length > 0) {
      console.log('👥 Inserting candidates...');
      for (const candidate of cleanDefaultData.candidates) {
        await connection.execute(
          'INSERT IGNORE INTO candidates (id, name, positionId, created_at) VALUES (?, ?, ?, ?)',
          [
            sanitizeValue(candidate.id), 
            sanitizeValue(candidate.name), 
            sanitizeValue(candidate.positionId), 
            sanitizeValue(candidate.created_at)
          ]
        );
      }
    }
    
    // Insert voters
    if (cleanDefaultData.voters.length > 0) {
      console.log('🗳️ Inserting voters...');
      for (const voter of cleanDefaultData.voters) {
        await connection.execute(
          'INSERT IGNORE INTO voters (name, email, studentId, hasVoted, departmentId, courseId, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
          [
            sanitizeValue(voter.name), 
            sanitizeValue(voter.email), 
            sanitizeValue(voter.studentId), 
            sanitizeValue(voter.hasVoted), 
            sanitizeValue(voter.departmentId), 
            sanitizeValue(voter.courseId), 
            sanitizeValue(voter.created_at)
          ]
        );
      }
    }
    
    // Insert elections
    if (cleanDefaultData.elections.length > 0) {
      console.log('🗳️ Inserting elections...');
      for (const election of cleanDefaultData.elections) {
        await connection.execute(
          'INSERT IGNORE INTO elections (id, title, description, startTime, endTime, status, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          [
            sanitizeValue(election.id), 
            sanitizeValue(election.title), 
            sanitizeValue(election.description), 
            sanitizeValue(election.startTime), 
            sanitizeValue(election.endTime), 
            sanitizeValue(election.status), 
            sanitizeValue(election.created_by), 
            sanitizeValue(election.created_at)
          ]
        );
      }
    }
    
    // Insert election_positions
    if (cleanDefaultData.election_positions.length > 0) {
      console.log('📋 Inserting election positions...');
      for (const ep of cleanDefaultData.election_positions) {
        await connection.execute(
          'INSERT IGNORE INTO election_positions (id, electionId, positionId, created_at) VALUES (?, ?, ?, ?)',
          [
            sanitizeValue(ep.id), 
            sanitizeValue(ep.electionId), 
            sanitizeValue(ep.positionId), 
            sanitizeValue(ep.created_at)
          ]
        );
      }
    }
    
    // Insert election_candidates
    if (cleanDefaultData.election_candidates.length > 0) {
      console.log('👥 Inserting election candidates...');
      for (const ec of cleanDefaultData.election_candidates) {
        await connection.execute(
          'INSERT IGNORE INTO election_candidates (id, electionId, candidateId, created_at) VALUES (?, ?, ?, ?)',
          [
            sanitizeValue(ec.id), 
            sanitizeValue(ec.electionId), 
            sanitizeValue(ec.candidateId), 
            sanitizeValue(ec.created_at)
          ]
        );
      }
    }
    
    console.log('✅ Clean default data seeding completed successfully!');
    
  } catch (error) {
    console.error('❌ Error seeding clean data:', error);
    throw error;
  }
}

// Create the clean seed data file
const cleanSeedDataPath = path.join(__dirname, 'clean-seed-data.js');
const cleanSeedDataContent = `// Clean Default Data for Voting System
// Generated on: ${new Date().toISOString()}

export const cleanDefaultData = ${JSON.stringify(cleanDefaultData, null, 2)};

// Helper function to handle undefined values
function sanitizeValue(value) {
  return value === undefined ? null : value;
}

export async function seedWithCleanData(connection) {
  console.log('🌱 Seeding database with clean default data...');
  
  try {
    // Insert admins
    if (cleanDefaultData.admins.length > 0) {
      console.log('👨‍💼 Inserting admins...');
      for (const admin of cleanDefaultData.admins) {
        await connection.execute(
          'INSERT IGNORE INTO admins (id, username, email, password, created_at) VALUES (?, ?, ?, ?, ?)',
          [
            sanitizeValue(admin.id), 
            sanitizeValue(admin.username), 
            sanitizeValue(admin.email), 
            sanitizeValue(admin.password), 
            sanitizeValue(admin.created_at)
          ]
        );
      }
    }
    
    // Insert departments
    if (cleanDefaultData.departments.length > 0) {
      console.log('🏢 Inserting departments...');
      for (const dept of cleanDefaultData.departments) {
        await connection.execute(
          'INSERT IGNORE INTO departments (id, name, created_by, created_at) VALUES (?, ?, ?, ?)',
          [
            sanitizeValue(dept.id), 
            sanitizeValue(dept.name), 
            sanitizeValue(dept.created_by), 
            sanitizeValue(dept.created_at)
          ]
        );
      }
    }
    
    // Insert courses
    if (cleanDefaultData.courses.length > 0) {
      console.log('📚 Inserting courses...');
      for (const course of cleanDefaultData.courses) {
        await connection.execute(
          'INSERT IGNORE INTO courses (id, name, departmentId, created_by, created_at) VALUES (?, ?, ?, ?, ?)',
          [
            sanitizeValue(course.id), 
            sanitizeValue(course.name), 
            sanitizeValue(course.departmentId), 
            sanitizeValue(course.created_by), 
            sanitizeValue(course.created_at)
          ]
        );
      }
    }
    
    // Insert positions
    if (cleanDefaultData.positions.length > 0) {
      console.log('🏛️ Inserting positions...');
      for (const position of cleanDefaultData.positions) {
        await connection.execute(
          'INSERT IGNORE INTO positions (id, name, voteLimit, displayOrder, created_at) VALUES (?, ?, ?, ?, ?)',
          [
            sanitizeValue(position.id), 
            sanitizeValue(position.name), 
            sanitizeValue(position.voteLimit || 1), 
            sanitizeValue(position.displayOrder || 0), 
            sanitizeValue(position.created_at)
          ]
        );
      }
    }
    
    // Insert candidates
    if (cleanDefaultData.candidates.length > 0) {
      console.log('👥 Inserting candidates...');
      for (const candidate of cleanDefaultData.candidates) {
        await connection.execute(
          'INSERT IGNORE INTO candidates (id, name, positionId, created_at) VALUES (?, ?, ?, ?)',
          [
            sanitizeValue(candidate.id), 
            sanitizeValue(candidate.name), 
            sanitizeValue(candidate.positionId), 
            sanitizeValue(candidate.created_at)
          ]
        );
      }
    }
    
    // Insert voters
    if (cleanDefaultData.voters.length > 0) {
      console.log('🗳️ Inserting voters...');
      for (const voter of cleanDefaultData.voters) {
        await connection.execute(
          'INSERT IGNORE INTO voters (name, email, studentId, hasVoted, departmentId, courseId, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
          [
            sanitizeValue(voter.name), 
            sanitizeValue(voter.email), 
            sanitizeValue(voter.studentId), 
            sanitizeValue(voter.hasVoted), 
            sanitizeValue(voter.departmentId), 
            sanitizeValue(voter.courseId), 
            sanitizeValue(voter.created_at)
          ]
        );
      }
    }
    
    // Insert elections
    if (cleanDefaultData.elections.length > 0) {
      console.log('🗳️ Inserting elections...');
      for (const election of cleanDefaultData.elections) {
        await connection.execute(
          'INSERT IGNORE INTO elections (id, title, description, startTime, endTime, status, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          [
            sanitizeValue(election.id), 
            sanitizeValue(election.title), 
            sanitizeValue(election.description), 
            sanitizeValue(election.startTime), 
            sanitizeValue(election.endTime), 
            sanitizeValue(election.status), 
            sanitizeValue(election.created_by), 
            sanitizeValue(election.created_at)
          ]
        );
      }
    }
    
    // Insert election_positions
    if (cleanDefaultData.election_positions.length > 0) {
      console.log('📋 Inserting election positions...');
      for (const ep of cleanDefaultData.election_positions) {
        await connection.execute(
          'INSERT IGNORE INTO election_positions (id, electionId, positionId, created_at) VALUES (?, ?, ?, ?)',
          [
            sanitizeValue(ep.id), 
            sanitizeValue(ep.electionId), 
            sanitizeValue(ep.positionId), 
            sanitizeValue(ep.created_at)
          ]
        );
      }
    }
    
    // Insert election_candidates
    if (cleanDefaultData.election_candidates.length > 0) {
      console.log('👥 Inserting election candidates...');
      for (const ec of cleanDefaultData.election_candidates) {
        await connection.execute(
          'INSERT IGNORE INTO election_candidates (id, electionId, candidateId, created_at) VALUES (?, ?, ?, ?)',
          [
            sanitizeValue(ec.id), 
            sanitizeValue(ec.electionId), 
            sanitizeValue(ec.candidateId), 
            sanitizeValue(ec.created_at)
          ]
        );
      }
    }
    
    console.log('✅ Clean default data seeding completed successfully!');
    
  } catch (error) {
    console.error('❌ Error seeding clean data:', error);
    throw error;
  }
}
`;

fs.writeFileSync(cleanSeedDataPath, cleanSeedDataContent);
console.log(`✅ Clean seed data file created: ${cleanSeedDataPath}`);

console.log('\n📊 Clean Default Data Summary:');
console.log('   - 4 Admins (including superadmin)');
console.log('   - 12 Departments');
console.log('   - 12 Courses');
console.log('   - 7 Positions');
console.log('   - 9 Candidates');
console.log('   - 5 Voters');
console.log('   - 1 Sample Election (for testing)');
console.log('   - 7 Election Positions');
console.log('   - 9 Election Candidates');

console.log('\n🔑 Default Login Credentials:');
console.log('   Superadmin: superadmin / password123');
console.log('   Admin: admin1123 / password123');

console.log('\n📝 Next steps:');
console.log('1. Update database.js to use clean-seed-data.js');
console.log('2. Test the clean default data');
console.log('3. Admins can now create their own elections manually'); 