...
const Results = () => {
  ...
  const analyticsData = useMemo(() => {
    ...
    const departmentData = departmentStats.map(dept => ({
      label: dept.name,
      value: dept.voterCount || 0
    }));

    // Course stats
    const courseData = courseStats.map(course => ({
      label: course.name,
      value: course.voterCount || 0
    }));
  ...
  }, [resultsData, realTimeStats, voteTimeline, departmentStats, courseStats]);
  ...
  return (
    <div className="results-container">
      ...
      <div className="stats-grid">
        <StatsCard title="Voters by Department" icon="fas fa-graduation-cap">
          {analyticsData.departmentData?.map((dept, index) => (
            <StatsItem key={index} label={dept.label} value={dept.value} />
          ))}
        </StatsCard>
        <StatsCard title="Voters by Course" icon="fas fa-book">
          {analyticsData.courseData?.map((course, index) => (
            <StatsItem key={index} label={course.label} value={course.value} />
          ))}
        </StatsCard>
      </div>
      ...
      <div className="tab-content">
            ...
            <div className="charts-grid">
              <div className="chart-card">
                <BarChart 
                  ...
                  title="Leading Votes per Position"
                />
              </div>
              <div className="chart-card">
                <BarChart 
        ...
        {activeTab === 'users' && (
          <div className="users-tab">
            <div className="stats-grid">
              <StatsCard title="User Statistics" icon="fas fa-user-friends">
                <StatsItem label="Total Users" value={userStats.totalUsers || 0} />
                <StatsItem label="Registered Voters" value={userStats.registeredVoters || 0} />
                <StatsItem label="Voted" value={userStats.voted || 0} />
                <StatsItem label="Not Voted" value={userStats.notVoted || 0} />
              </StatsCard>
              <StatsCard title="Voting Activity" icon="fas fa-chart-line">
                <DonutChart 
                  data={[
                    { label: "Voted", value: userStats.voted || 0 },
                    { label: "Not Voted", value: userStats.notVoted || 0 }
                  ]}
